...
MAC_MODEL_MAP = {
    "02:A1:01:16:3D:5C": "L0002",
    "02:A1:01:16:3D:5D": "L0004",
    "02:A1:01:16:3D:5E": "L0016",
    "02:A1:01:16:3D:5F": "L0032",
    "02:A1:01:16:3D:60": "L0064",
    "02:A1:01:16:3D:61": "L0128",
    "02:A1:01:16:3D:62": "L0256",
    "02:A1:01:16:3D:63": "L0512",
}
    ...
    try:
        ...
        if r.status_code == 200:
            try:
                j = r.json()
                if any(
                    k in j for k in ["dev_class", "total_locker", "c_name", "mac_addr"]
                ):
                    mac = j.get("mac_addr", "?")
...
def scan_subnet(subnet: str, port: int, workers: int = 150):
    ...
    print(
        f"  {BLUE}Scanning {subnet}.1 – {subnet}.254 port {port} ({total_hosts} hosts)...{RESET}\n"
    )
...
def print_controller(c: dict, idx: int):
    ...
    used = (
        (int(total) - int(avail))
        if str(total).isdigit() and str(avail).isdigit()
        else 0
    )
...
def deep_check(c: dict, api_key: str = "locker-secret-123"):
    ...
    for method, path, body, auth, expect in checks:
        ...
        try:
            ...
            mark = "✓" if ok else "✗"
            print(
                f"  {color}{mark}{RESET} {method:<7} {path:<22} → {r.status_code} (expect {expect})"
            )
            if not ok:
                all_ok = False
    ...
    status = (
        f"{GREEN}{BOLD}Semua OK!{RESET}"
        if all_ok
        else f"{YELLOW}Ada anomali — cek kode Arduino{RESET}"
    )
    print(f"\n  {status}")
...
def save_result(controllers: list) -> str:
    ...
    data = {
        ...
        "controllers": [
            {k: v for k, v in c.items() if k != "raw"} for c in controllers
        ],
    }
...
def main():
    ...
    parser.add_argument(
        "--ip", type=str, help="IP target (local/public), e.g. 93.144.178.16"
    )
    ...
    parser.add_argument(
        "--ports",
        type=str,
        default="8000,80,8080,3000",
        help="Port kandidat saat --ip mode",
    )
    parser.add_argument(
        "--deep", action="store_true", help="Deep check endpoint setelah ditemukan"
    )
    ...
    parser.add_argument(
        "--workers", type=int, default=150, help="Thread workers (default: 150)"
    )
